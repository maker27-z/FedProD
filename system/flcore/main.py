...
from flcore.servers.serverprotoavg import FedProtoAvg
from flcore.servers.serverprotoavgwithgood import FedProtoAvgWithGood
...
from flcore.servers.serverSentinel import serverSentinel
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            ...
            print(args.with_dropout)
            args.teacher_model = StrongTeacherMLP(num_features=args.feature_dim, num_classes=args.num_classes).to(args.device)

            if args.with_dropout:
                args.model = resnet10(num_classes=args.num_classes, with_drop=True).to(args.device)
            else:
                args.model = resnet10(num_classes=args.num_classes, with_drop=False).to(args.device)
            args.student_model = copy.deepcopy(args.model)
        elif model_str == "ResNet4":
            print(args.with_dropout)
            if args.with_dropout:
                args.model = resnet4(num_classes=args.num_classes, with_drop=True).to(args.device)
            else:
                args.model = resnet4(num_classes=args.num_classes, with_drop=False).to(args.device)
        ...
        if args.algorithm == "FedAvg":
        ...
        elif args.algorithm == "FedProtoAvg":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedProtoAvg(args, i)

        elif args.algorithm == "FedProtoAvgWithGood":
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedProtoAvgWithGood(args, i)
            ...
            # For teacher model (StrongTeacherMLP)
            if hasattr(args.teacher_model, 'classifier'):
                # Enhanced teacher model case
                teacher_head = copy.deepcopy(args.teacher_model.classifier)
                teacher_base = copy.deepcopy(args.teacher_model.feature_extractor)
                
                # Transfer attention and projection if present
                if hasattr(args.teacher_model, 'attention'):
                    teacher_base.attention = copy.deepcopy(args.teacher_model.attention)
                
                if hasattr(args.teacher_model, 'projection'):
                    teacher_base.projection = copy.deepcopy(args.teacher_model.projection)
                    
                args.teacher_model = BaseHeadSplit(teacher_base, teacher_head)
            else:
                # Original case
                teacher_head = copy.deepcopy(args.teacher_model.fc)
                args.teacher_model.fc = torch.nn.Identity()
                args.teacher_model = BaseHeadSplit(args.teacher_model, teacher_head)

            # For student model (AmazonMLP)
            student_head = copy.deepcopy(args.student_model.fc)
            args.student_model.fc = torch.nn.Identity()
            args.student_model = BaseHeadSplit(args.student_model, student_head)

            print("Teacher model:")
            print(args.teacher_model)
            print("Student model:")
            print(args.student_model)
            server = serverSentinel(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-fd', "--feature_dim", type=int, default=16)
    ...
    parser.add_argument('-d_alpha', "--d_alpha", type=float, default=0.7)
    parser.add_argument('-alpha_param', "--alpha_param", type=float, default=0.1)
    parser.add_argument('-beta_param', "--beta_param", type=float, default=0.1)
    parser.add_argument('-T', '--temp', type=float, default=2)
    parser.add_argument('-wp', '--warm_up', type=int, default=10)
    parser.add_argument('-dp', '--with_dropout', type=int, default=0)
    parser.add_argument('-th', '--threshold', type=float, default=1.2)
    parser.add_argument('-glrt','--global_learning_rate', type=float, default=0.01)
    parser.add_argument('-seed', '--seed', type=int, default=0)
    parser.add_argument('-isP', '--isPositive', type=int, default=1)
    parser.add_argument('-isN', '--isNegative', type=int, default=1)
    
        # Sentinel
    # parser.add_argument('-mlr', "--mentee_learning_rate", type=float, default=0.0001)
    # parser.add_argument('-Ts', "--T_start", type=float, default=0.95)
    # parser.add_argument('-Te', "--T_end", type=float, default=0.98)
