...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)  # 确保展平操作统一

        if self.with_drop:
            y_drop = self.dropout(x)
            y_drop = self.fc(y_drop)
            x = self.fc(x)
            return y_drop, x
        else:
            x = self.fc(x)
...
if __name__ == "__main__":
    model = resnet10(with_drop=True,num_classes=15)
    # print(model)
    data = torch.randn(1, 1, 1, 49)
    # output = model(data)
    # print(output)
    ...
    model.train()
    for i in range(10):
        proto, x = model.base(data)
        print(proto.shape)
        # drop_logits,_ = model.base(data)
        # y_drop = model.head(drop_logits)
        # print(y_drop)
